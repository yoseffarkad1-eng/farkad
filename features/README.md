...
| `organization` | v103, served: the app put in order. The week's empty screen names the next move (the one thing a person sees); three button classes that meant nothing, including `.btn-primary`, which matched no rule at all; the rules review, which found no gap and now names which check answers which claim; and the documents - the Arabic guide, thirteen acceptance rows and the order to run them in, and the rules-first deploy. The ⋯ panel was checked and NOT reordered: it was already right. | contract, handoff |
| `held` | v105: the held records, shown. A write that lost a race while the phone was away was counted and never named; the ⋯ panel now lists each one with both sides — this phone's value and the cloud's, in the day screen's words — and the two answers, each a fresh explicit edit of the same path. Came from the owner's six held cells of one Thursday, named first from a rescue export; the hunt that found the second window. | findings, handoff |
...
| `next` | The briefs written for whoever comes next. `brief-for-opus.md` (the eight items after v98) is **done** — see `false-holds`. `opus-closeout.md` is the wave plan this line is working through; its Wave 0 shipped as v99, its Wave 1 (both halves) as v100, its Wave 2 as v101, its Wave 3 as v102 and its Wave 4 as v103; Wave 5's two deliverables shipped early, in v100. | briefs |
