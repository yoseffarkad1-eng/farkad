...
| `held` | v105: the held records, shown. A write that lost a race while the phone was away was counted and never named; the ⋯ panel now lists each one with both sides — this phone's value and the cloud's, in the day screen's words — and the two answers, each a fresh explicit edit of the same path. Came from the owner's six held cells of one Thursday, named first from a rescue export. | handoff |
