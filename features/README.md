...
| `day-room` | v101: room on the day screen without moving a bar. The header compacts to one row on scroll (157px of chrome above the first name becomes about 57), and the site cards' two footer buttons become icon buttons on the coloured head — a row back per card. The third change, moving the switcher into the header, is the one worth reading about: measured, it gives back **three pixels, not a row**, and the handoff says why and what it buys instead. | contract, handoff |
...
| `next` | The briefs written for whoever comes next. `brief-for-opus.md` (the eight items after v98) is **done** — see `false-holds`. `opus-closeout.md` is the wave plan this line is working through; its Wave 0 shipped as v99, its Wave 1 (both halves) as v100 and its Wave 2 as v101. | briefs |
